...
from attacker_evaluator import AttackerEvaluator
...
def _train(path_to_train_tfrecords_file, num_train_examples, path_to_val_tfrecords_file, num_val_examples,
    ...
    with tf.Graph().as_default():
        ...
        recovered_ssim = tf.reduce_mean(tf.abs(tf.image.ssim(tf.image.rgb_to_grayscale(image_batch), tf.image.rgb_to_grayscale(recovered), max_val=2)))
        ...
        attacker_op = optimizer.minimize(attacker_loss, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,scope='attacker'), global_step=global_step)
        ...
        with tf.Session() as sess:
            ...
            evaluator = AttackerEvaluator(os.path.join(path_to_train_log_dir, 'eval/val'))
            ...
            attacker_saver = tf.train.Saver(var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,scope='attacker'))
            assert tf.train.checkpoint_exists(path_to_restore_model_checkpoint_file), \
                '%s not found' % path_to_restore_model_checkpoint_file
            model_saver.restore(sess, path_to_restore_model_checkpoint_file)
            print('Model restored from file: %s' % path_to_restore_model_checkpoint_file)
            ...
            while True:
                ...
                _, attacker_loss_val, summary_val, global_step_val, learning_rate_val = sess.run([attacker_op, attacker_loss, summary, global_step, learning_rate])
                ...
                if global_step_val % num_steps_to_show_loss == 0:
                    ...
                    print('=> %s: step %d, loss = %f (%.1f examples/sec)' % (
                        datetime.now(), global_step_val, attacker_loss_val, examples_per_sec))
                ...
                path_to_latest_attacker_checkpoint_file = attacker_saver.save(sess, os.path.join(path_to_train_log_dir, 'attacker.ckpt'))
                accuracy = evaluator.evaluate(path_to_restore_model_checkpoint_file, path_to_latest_attacker_checkpoint_file, path_to_val_tfrecords_file,
                                              num_val_examples,
                                              global_step_val)
